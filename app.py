...
def main():
    ...
    device_type = st.sidebar.selectbox("Device Type", ["Camera", "Laptop", "Phone"])
...
def camera_preferences():
    budget = st.sidebar.number_input("Budget (USD)", min_value=100, max_value=10000, value=1000, step=100)
    usage = st.sidebar.selectbox("Usage", ["Photography", "Vlogging", "Video Editing", "Gaming", "Other"])
    ...
    return budget, usage, camera_type, sensor_size, resolution, lens_type, lens_focal_length, lens_aperture
def laptop_preferences():
    budget = st.sidebar.number_input("Budget (USD)", min_value=500, max_value=10000, value=1000, step=100)
    usage = st.sidebar.selectbox("Usage", ["Work", "Gaming", "Design", "Other"])
    ...
    screen_size = st.sidebar.number_input("Screen Size (inches)", min_value=13.0, max_value=17.0, value=15.0, step=0.5)
    screen_resolution = st.sidebar.selectbox("Screen Resolution", ["1920x1080", "2560x1440", "3840x2160"])
    return budget, usage, processor, memory, storage, screen_size, screen_resolution
def phone_preferences():
    budget = st.sidebar.number_input("Budget (USD)", min_value=200, max_value=10000, value=500, step=100)
    usage = st.sidebar.selectbox("Usage", ["Work", "Gaming", "Design", "Other"],key="phone_usage")
    screen_size = st.sidebar.number_input("Screen Size (inches)", min_value=5.0, max_value=7.0, value=6.0, step=0.5)
    ...
    battery_capacity = st.sidebar.number_input("Battery Capacity (mAh)", min_value=2000, max_value=10000, value=3000, step=100) 
...
#     lens_aperture = st.sidebar.number_input("Aperture (f/)", min_value=1.8, max_value=16.0, value=4.0, step=0.1)
    ...
    # Laptop preferences
    laptop_budget = st.sidebar.number_input("Budget (USD)", min_value=500, max_value=10000, value=1000, step=100)
    usage = st.sidebar.selectbox("Usage", ["Work", "Gaming", "Design", "Other"])
    processor = st.sidebar.selectbox("Processor", ["Intel Core i3", "Intel Core i5", "Intel Core i7", "Intel Core i9", "AMD Ryzen 3", "AMD Ryzen 5", "AMD Ryzen 7", "AMD Ryzen 9"])
    memory = st.sidebar.selectbox("Memory (GB)", [4, 8, 16, 32, 64])
    storage = st.sidebar.selectbox("Storage (GB)", [256, 512, 1024, 2048, 4096])
    ...
    # Phone preferences
    phone_budget = st.sidebar.number_input("Budget (USD)", min_value=200, max_value=10000, value=500, step=100)
    usage = st.sidebar.selectbox("Usage", ["Work", "Gaming", "Design", "Other"],key="phone_usage")
    screen_size = st.sidebar.number_input("Screen Size (inches)", min_value=5.0, max_value=7.0, value=6.0, step=0.5)
    screen_resolution = st.sidebar.selectbox("Screen Resolution", ["1080x1920", "1440x2560", "2160x3840"])
    storage = st.sidebar.selectbox("Storage (GB)", [128, 256, 512, 1024])
